...
def parse_eventos(payload):
    ...
    Retorna lista de dicts com a ordem home/away da fonte:
      {time1, gols1, time2, gols2, time1_pt, time2_pt, completo}
    (time*_pt = nome PT da planilha ou None se não mapeado.)
    """
    partidas = []
...
def buscar_partidas_finalizadas(datas, baixar=_baixar):
    """Lista (time1_pt, time2_pt, gols1, gols2) de jogos FINALIZADOS e mapeados.
    ...
    for d in datas:
        ...
        for p in parse_eventos(payload):
            ...
            if (p["time1_pt"] and p["time2_pt"]
                ...
                out.append((p["time1_pt"], p["time2_pt"], p["gols1"], p["gols2"]))
    if nao_mapeados:
